...
export class ShipmentsService {
  ...
  update(id: number,shipment: any) {
    const url = `${base_url}admin/shipments/${id}/`;
    return this.http.patch(url,shipment, this.headers)
  }
  delete(shipmentId: number) {
    const url = `${base_url}admin/shipments/${shipmentId}`;
  ...
  getDollys() {
    const url = `${base_url}admin/dollys/`;
    return this.http.get(url, this.headers);
  }
